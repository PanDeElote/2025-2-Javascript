...
function diffDays(actualDate, birthDate) {
  ...
  const edadEnDias = Math.floor(diff / (1000 * 60 * 60 * 24 /*Para meses y el año  * 30 * 12*/));
...
async function main() {
  ...
  const edadEnMeses = diffDays(actualDate, birthDate) / 30;
  const edadEnAños = diffDays(actualDate, birthDate) / 365;
  console.log(`${edadEnMeses} meses`);
  console.log(`${edadEnAños} años`);
